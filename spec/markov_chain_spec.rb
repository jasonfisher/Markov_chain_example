...
#makes no sense that a and b should be created by calling increment_probability with "start" and "end"
...
#instead might read:
  ...
  # test probability of outcome based on weights with larger disparity
  it "should choose nodes in random_walk randomly by weight" do
    build_graph(:b => 1, :a => 10)
    walks = (1..200).map{|i| @mc.random_walk("start")}    
    middle_steps = walks.map{|x| x[1]}
    (middle_steps.count("a") / middle_steps.count("b")).should > 5 #conservative guesstimate, really averages to 10
  end
